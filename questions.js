...
const mainQuestions = [
  ...
  },
  {
    ...
    when(answers) {
      return answers.interest && !answers.moreItems;
    },
  },
];

const addMoreItemsQuestions = [
  {
    type: "list",
    name: "products",
    message(answers) {
      answers.interest = true;
      return "Which product would like to buy";
    },
    choices: products().map((a) => a.name),
    filter(value) {
      return products().filter((a) => a.name === value)[0];
...
const confirmCartQuestions = [
  ...
  },
  {
    type: "number",
    name: "amountPaid",
    message: "Please pay total amount for the products",
    validate(value) {
